...
const excludeList = ["khronos-MetalRoughSpheres-LDR", "khronos-BoxInterleaved"];
...
(async () => {
	...
	const req = await fetch(
		"https://raw.githubusercontent.com/google/model-viewer/master/packages/render-fidelity-tools/test/config.json"
	);
...
async function saveScreenshot(scenario, targetFolder) {
	...
	await page.goto(
		`http://localhost:1234/viewerTest.html?hideUI=true&tiles=1&samples=${SAMPLES}#${name}`
	);
		...
		await page.evaluate(() => {
			return new Promise((resolve, reject) => {
				...
				const handle = setTimeout(() => {
					reject(
						new Error(`Failed to render in ${(1e-3 * TIMEOUT).toFixed(2)}s.`)
					);
				}, TIMEOUT);
